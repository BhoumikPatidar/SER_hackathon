...
Relocator::Result eld::relocTPOFF32(Relocation &pReloc, x86_64Relocator &pParent,
  ...
  // Get TLS template size from backend
  uint64_t TLSTemplateSize = pParent.getTarget().getTLSTemplateSize();
  
  // S = TLS symbol value (offset within TLS template)
  uint64_t S = pParent.getSymValue(&pReloc);
  Relocator::DWord A = pReloc.addend();
  
  // For x86_64 TLS Variant II: TPOFF = S + A - TLS_Template_Size
  // This produces a negative offset from thread pointer to TLS variable
  // Cast to signed since TPOFF32 is a signed relocation
